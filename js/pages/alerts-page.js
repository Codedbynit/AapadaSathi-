...
export class AlertsPage {
    ...
    container.innerHTML = `
      <div class="alerts-page-container fade-in">
        ...
        <div class="glass-panel alerts-filter-bar">
          <div>
            <h2>Active Disaster Warnings</h2>
            <p style="font-size:0.875rem;">Verified settlement-scale early warnings prioritized by urgency.</p>
          </div>
          ...
          <div style="display:flex; align-items:center; gap:0.75rem; flex-wrap:wrap;">
            ...
            <div class="map-filters-group">
              <button class="filter-chip active alert-filter-chip" data-severity="ALL">All</button>
              <button class="filter-chip alert-filter-chip" data-severity="CRITICAL">Critical</button>
              <button class="filter-chip alert-filter-chip" data-severity="HIGH">High</button>
              <button class="filter-chip alert-filter-chip" data-severity="MODERATE">Moderate</button>
            </div>
          </div>
        </div>
        ...
        <div class="alerts-feed-grid" id="alerts-feed-list">
          ${this.generateAlertCardsHtml(alerts)}
        </div>
        ...
        <div id="modal-backdrop" style="display:none; position:fixed; top:0; left:0; width:100%; height:100%; background:rgba(0,0,0,0.7); backdrop-filter:blur(6px); z-index:1050;"></div>
      </div>
    `;

    this.attachEvents(alerts);
  }

  static generateAlertCardsHtml(alerts) {
    if (!alerts || alerts.length === 0) {
      return `
        <div class="glass-panel state-container">
          <div class="state-icon"><i class="fa-solid fa-bell-slash"></i></div>
          <h3 class="state-title">No Active Warnings Match Your Filter</h3>
          <p class="state-desc">All monitored hydrological levels in this category are within nominal threshold parameters.</p>
        </div>
      `;
    }
    ...
    return alerts.map(alert => {
      ...
      return `
        <div class="glass-panel alert-item-card severity-${alert.severity.toLowerCase()} ${isRead ? 'alert-read' : ''}" data-id="${alert.id}">
          <div class="alert-icon-col" style="color:${alert.severity === 'CRITICAL' ? '#f87171' : '#fb923c'};">
          ...
          <div class="alert-content-col">
            <div class="alert-title-row">
              ...
              ${isRead ? '<span style="font-size:0.75rem; color:#94a3b8;"><i class="fa-solid fa-check"></i> Acknowledged</span>' : ''}
            </div>
            <p class="alert-description">${alert.message}</p>
            <div class="alert-meta-row">
              <span><i class="fa-solid fa-location-dot"></i> ${alert.settlementName}</span>
              <span><i class="fa-solid fa-clock"></i> Issued: ${alert.issuedAt}</span>
              <span><i class="fa-solid fa-hourglass-start"></i> Lead Time: <strong>${alert.leadTimeHours} hrs</strong></span>
              <span><i class="fa-solid fa-chart-pie"></i> Confidence: ${alert.confidence}</span>
            </div>
          </div>
          ...
          <div class="alert-actions-col">
            ...
            <button class="btn btn-secondary btn-sm btn-ack-alert" data-id="${alert.id}">
              <i class="fa-solid fa-check"></i> ${isRead ? 'Done' : 'Acknowledge'}
            </button>
          </div>
        </div>
      `;
  ...
  static attachEvents(allAlerts) {
    ...
    const filterAndRender = () => {
      ...
      feedList.innerHTML = this.generateAlertCardsHtml(filtered);
      this.attachCardActions(allAlerts);
    };
  ...
  static attachCardActions(allAlerts) {
    // Acknowledge button
    document.querySelectorAll('.btn-ack-alert').forEach(btn => {
      btn.addEventListener('click', (e) => {
        ...
        Toast.show('Alert marked as acknowledged.', 'info', 2000);
      ...
      btn.addEventListener('click', (e) => {
        ...
        modalContent.innerHTML = `
          <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:1rem;">
            <span class="status-badge ${RiskService.getRiskBadgeClass(alert.severity)}">${alert.severity}</span>
            <button id="btn-close-modal" style="background:none; border:none; color:#ffffff; font-size:1.25rem; cursor:pointer;"><i class="fa-solid fa-xmark"></i></button>
          </div>
          <h3 style="color:#ffffff; margin-bottom:0.75rem;">${alert.title}</h3>
          <p style="color:#cbd5e1; font-size:0.95rem; line-height:1.5; margin-bottom:1.25rem;">${alert.message}</p>
          <div style="padding:0.85rem; background:rgba(255,255,255,0.06); border-radius:8px; font-size:0.85rem; margin-bottom:1.25rem;">
            <div><strong>Location:</strong> ${alert.settlementName}</div>
            <div><strong>Issued:</strong> ${alert.issuedAt}</div>
            <div><strong>Expected Impact:</strong> ${alert.expectedImpactAt}</div>
            <div><strong>Lead Time:</strong> ${alert.leadTimeHours} hrs</div>
            <div><strong>Confidence:</strong> ${alert.confidence}</div>
          </div>
          <div style="display:flex; gap:0.75rem; justify-content:flex-end;">
            <button class="btn btn-secondary btn-sm" id="btn-modal-dismiss">Close</button>
            ${alert.evacuationRequired ? `
              <button class="btn btn-danger btn-sm" id="btn-modal-evacuate">
                <i class="fa-solid fa-person-walking-arrow-right"></i> Open Safe Route
              </button>
            ` : ''}
          </div>
        `;
